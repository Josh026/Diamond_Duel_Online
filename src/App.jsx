...
// Minimal Supabase REST + Realtime client (no npm package needed)
const supa = {
  async getRoom(code) {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/rooms?code=eq.${code}&select=state`, {
  ...
  subscribeRoom(code, callback) {
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (msg.event === "postgres_changes" && msg.payload?.data?.record?.state) {
          callback(msg.payload.data.record.state);
        }
      } catch {}
    };
    ws.onerror = () => {};
...
export default function BaseballGame(){
  ...
  // Online mode — write state when it's explicitly our turn (_turn matches our side)
  useEffect(()=>{
    if(g.mode!=="online"||!roomCode||onlineState!=="playing") return;
    if(!g._turn) return;
    if(g._turn !== onlineSide) return; // only write when it's our turn
    supa.upsertRoom(roomCode, g).catch(()=>{});
  },[g, roomCode, onlineState, onlineSide]);

  // Online mode — subscribe and apply opponent state when it's their turn
  useEffect(()=>{
    if(onlineState!=="playing"||!roomCode) return;
    const unsub = supa.subscribeRoom(roomCode, (remoteState)=>{
      if(!remoteState||!remoteState._turn) return;
      setG(prev=>{
        // Only apply if the remote state is authored by our opponent
        if(remoteState._turn === onlineSide) return prev; // that's our own write echoing back
        // Preserve our playerSide and onlineSide — everything else comes from remote
        return{...remoteState, playerSide:prev.playerSide};
      });
    });
  ...
  function playerDraftPick(player){
    setG(prev=>{
      ...
      if(done){
        const allTaken=[...newR.batters,...newR.pitchers,...prev.aiRoster.batters,...prev.aiRoster.pitchers].map(p=>p.id);
        const filledPlayer=autoFillRoster(newR,prev.draftPool,allTaken,FULL_BATTERS,FULL_PITCHERS);
        const takenAfter=[...filledPlayer.batters,...filledPlayer.pitchers,...prev.aiRoster.batters,...prev.aiRoster.pitchers].map(p=>p.id);
        const filledAI=autoFillRoster(prev.aiRoster,prev.draftPool,takenAfter,FULL_BATTERS,FULL_PITCHERS);
        const afterDraftTurn = prev.mode==="online"?"away":"away";
        return{...prev,playerRoster:filledPlayer,aiRoster:filledAI,draftPick:next,draftOptions:[],
          phase:"lineup_setup",_turn:afterDraftTurn,gameLog:[`⚾ ${prev.teams.away} vs ${prev.teams.home}`,`--- Inning 1 (Top) ---`]};
      }
  ...
  function choosePitch(pitchId){setG(prev=>({...prev,pitchChosen:pitchId,phase:"batting",_turn:prev.topBottom==="top"?"away":"home"}));}
  ...
  function intentionalWalk(){
    setG(prev=>{
      ...
      return{
        ...
        phase:"result",_turn:battingSide,
        lastDice:null,
  ...
  function resolveOutcome(){
    setG(prev=>{
      ...
      const resultTurn = isWalkoff?"home":(newPhase==="gameover"?"away":battingSide);
      return{
  ...
  const playerBatting=g.mode==="vs-ai"&&(g.playerSide==="away"?g.topBottom==="top":g.topBottom==="bottom");
  const playerPitching=g.mode==="vs-ai"&&(g.playerSide==="away"?g.topBottom==="bottom":g.topBottom==="top");
  ...
  if(g.phase==="lineup_setup"||(lineupPhase==="ai"&&g.mode==="two-player")){
    ...
    const editRoster=editSide===g.playerSide?g.playerRoster:g.aiRoster;
    ...
    return(
      <div style={root}>
        <div style={{width:"100%",maxWidth:580,padding:"16px 16px 0",boxSizing:"border-box"}}>
          ...
          <div style={{fontSize:12,color:"#64748b",marginBottom:8}}>Your lineup cycles through all 9 batters in order. Put your best on-base players at the top, power hitters in the middle (3-5), and speed players lower.</div>
    ...
    <div style={root}>
      ...
      <div style={card}>
        ...
        {playerIsPitching
          ...
          :<div style={{color:"#64748b",textAlign:"center",padding:"20px 0"}}>⚾ AI is winding up…</div>
        }
      </div>
    </div>
  );
  ...
  if(g.phase==="batting")return(
    <div style={root}>
      ...
      <div style={card}>
        ...
        :<div style={{color:"#64748b",textAlign:"center",padding:"20px 0"}}>🤖 AI batter deciding…</div>}
      </div>
    </div>
  );
    ...
    return(
      <div style={root}>
        <div style={{...card,maxWidth:380,margin:"80px auto 0",textAlign:"center"}}>
          ...
          <div style={{fontWeight:800,fontSize:16,marginBottom:8}}>Waiting for opponent…</div>
          ...
          <div style={{fontSize:12,color:"#475569"}}>Share this code with your friend</div>
        </div>
      </div>
    );
  }
  ...
  if(g.phase==="online_lobby"){
    const isHost = onlineSide==="away";
    return(
      <div style={root}>
        <div style={{...card,maxWidth:420,margin:"60px auto 0",textAlign:"center"}}>
          <div style={{fontSize:32,marginBottom:8}}>🌐</div>
          <div style={{fontWeight:900,fontSize:18,marginBottom:4}}>Online Match</div>

          {onlineState==="idle"&&(
            <div>
              <div style={{fontSize:13,color:"#64748b",marginBottom:20}}>
                Play against a friend on another device
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                <button style={btn("#E63946")} onClick={async()=>{
                  ...
                  setRoomCode(code);
                  setOnlineSide("away");
                  // Create room with initial draft state
                  const pool = buildDraftPool();
                  const firstOptions = buildDraftOptions(pool,[],true,true,{S:0,A:0,B:0,C:0},3,new Set());
                  const initState = {
                    ...initGame(),
                    mode:"online",
                    phase:"online_wait",
                    _turn:"away", // away sets lineup first
                    playerSide:"away",
                    draftPool:pool,
                    draftPick:0,
                    draftOptions:firstOptions,
                  ...
                  await supa.upsertRoom(code, initState);
                  setG(initState);
                  setOnlineState("lobby");
                  // Poll until guest joins
                  const poll = setInterval(async()=>{
                    const remote = await supa.getRoom(code);
                    if(remote?.phase==="draft"&&remote?.playerSide==="home"){
                      clearInterval(poll);
                      setOnlineState("playing");
                      setG(prev=>({...remote,playerSide:"away"}));
                    }
                  },2000);
                ...
                <div style={{fontSize:13,color:"#475569"}}>— or —</div>
                <div style={{display:"flex",gap:8}}>
                  <input value={roomInput} onChange={e=>setRoomInput(e.target.value.toUpperCase())}
                    placeholder="Enter room code"
                    maxLength={5}
                    style={{flex:1,background:"#0f172a",border:"1px solid #334155",color:"#f8fafc",
                  ...
                  <button style={{...btn("#3b82f6"),width:"auto",padding:"10px 16px"}}
                    onClick={async()=>{
                      if(roomInput.length<3){setOnlineError("Enter a valid room code");return;}
                      setOnlineError("");
                      setOnlineState("joining");
                      const remote = await supa.getRoom(roomInput);
                      if(!remote){setOnlineError("Room not found");setOnlineState("idle");return;}
                      if(remote.phase!=="online_wait"){setOnlineError("Room already started");setOnlineState("idle");return;}
                      setRoomCode(roomInput);
                      setOnlineSide("home");
                      // Join as home team — trigger host to start
                      const pool = buildDraftPool();
                      // Guest joins — set _turn to "away" so host starts their draft
                      const homeState = {
                        ...remote,
                        playerSide:"home",
                        phase:"draft",
                      ...
                      await supa.upsertRoom(roomInput, homeState);
                      setOnlineState("playing");
                      setG({...homeState, playerSide:"home"});
                    }}>
                    Join
                  </button>
                </div>
          ...
          {onlineState==="creating"&&(
            <div style={{color:"#64748b",fontSize:13}}>Creating room…</div>
          )}

          {onlineState==="joining"&&(
            <div style={{color:"#64748b",fontSize:13}}>Joining room…</div>
          )}
          ...
          {onlineState==="lobby"&&(
            <div>
              ...
              <button style={btn("#475569")} onClick={()=>{
                setOnlineState("idle");setRoomCode("");
                setG(prev=>({...prev,phase:"menu"}));
              }}>Cancel</button>
            </div>
          )}
          ...
          {onlineState==="error"&&(
            <div>
              <div style={{color:"#f87171",marginBottom:12}}>{onlineError}</div>
              <button style={btn("#475569")} onClick={()=>{
                setOnlineState("idle");
                setG(prev=>({...prev,phase:"menu"}));
              }}>Back</button>
