...
const TEAM_NAMES = [
  ...
  "Timberwolves","Fireflies","Mudcats","JumboShrimp","Thunderbolts",
  "Rattlers","Pelicans","Zephyrs","YardGoats","Nighthawks",
  "Ospreys","TrashPandas","RumblePonies","Kernels","Nuts",
  ...
  if(g.phase==="rolling"){
    ...
    if(isOnline && !playerBatting){
      ...
      {debugOverlay}
          ...
          <div style={card}>
            <div style={{color:"#64748b",textAlign:"center",padding:"30px 0"}}>⏳ Waiting for opponent to reveal dice…</div>
          </div>
        </div>
      );
    }
  ...
  if(g.phase==="between"){
    ...
    if(isOnline && g.playerSide!=="away"){
      ...
      {debugOverlay}
          <Scoreboard g={g} onBoxScore={()=>setShowBoxScore(true)} onSoundToggle={soundOn?{on:true,fn:()=>{_soundEnabled=false;setSoundOn(false);}}:{on:false,fn:()=>{_soundEnabled=true;setSoundOn(true);}}}/>
          <div style={card}>
            <div style={{color:"#64748b",textAlign:"center",padding:"30px 0"}}>⏳ Waiting for opponent to continue…</div>
          </div>
        </div>
      );
    }
