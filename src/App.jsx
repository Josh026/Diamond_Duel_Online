...
export default function BaseballGame(){
  ...
  useEffect(()=>{
    ...
    const key = g.phase + g._turn + (g.count?.balls??0) + (g.count?.strikes??0) + (g.outs??0);
  ...
  useEffect(()=>{
    ...
    const poll = setInterval(async()=>{
      try{
        ...
        const remoteKey = remote.phase + remote._turn + (remote.count?.balls??0) + (remote.count?.strikes??0) + (remote.outs??0);
        if(remoteKey === lastSeenPhase) return;
        lastSeenPhase = remoteKey;
