...
const supa = {
  // Get full room row
  async getRoom(code) {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/rooms?code=eq.${encodeURIComponent(code)}&select=*`, {
      headers: supaHeaders
    });
  ...
  // Create or update specific fields in room
  async patchRoom(code, fields) {
    const existing = await supa.getRoom(code);
    const body = existing
  ...
  subscribeRoom(code, callback) {
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if(msg.event==="postgres_changes") callback(msg.payload?.data?.record);
      } catch{}
    };
    ...
    return(
      <div style={root}>
        <div style={{...card,maxWidth:420,margin:"60px auto 0",textAlign:"center"}}>
          ...
          {onlineState==="idle"&&(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <button style={btn("#E63946")} onClick={async()=>{
                ...
                // Away drafts first — fully local, no sync during draft
                const pool=buildDraftPool();
                const firstOptions=buildDraftOptions(pool,[],true,true,{S:0,A:0,B:0,C:0},3,new Set());
                setG({
                  ...initGame(),
          ...
          {(onlineState==="creating"||onlineState==="joining")&&(
            <div style={{color:"#64748b",padding:"20px 0"}}>
              {onlineState==="creating"?"Creating room…":"Joining room…"}
            </div>
          )}
