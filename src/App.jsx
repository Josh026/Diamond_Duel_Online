...
// Launch online game once both rosters are confirmed in Supabase
async function launchOnlineGame(roomCode, row, side, totalInnings, myRoster, oppRoster, myLineup, oppLineup, setOnlineState, setG, writerSide){
  const oppSide = side==="away"?"home":"away";
  // Shared game state (no rosters — each device keeps their own)
  const shared = {
    mode:"online", phase:"pitching", _turn:"home",
    totalInnings, status:"playing",
    teams:{away:row.awayName||"Away", home:row.homeName||"Home"},
    teamColors:{away:TEAM_PALETTE[row.awayColorIdx||0], home:TEAM_PALETTE[row.homeColorIdx||1]},
    score:{away:0,home:0}, inning:1, topBottom:"top", outs:0,
    count:{balls:0,strikes:0}, bases:[false,false,false],
    pitchHistory:[], isWalkoff:false,
    gameLog:[`⚾ Online Game · ${totalInnings} innings`,`--- Inning 1 (Top) ---`],
    activePitcherIdx:{away:0,home:0}, pitcherBF:{away:0,home:0},
    usedPitchers:{away:[],home:[]}, batterIndex:{away:0,home:0},
    // Include rosters and lineup in shared state so both devices can reconstruct
    awayRoster: side==="away"?myRoster:oppRoster,
    homeRoster: side==="home"?myRoster:oppRoster,
    awayLineup: side==="away"?myLineup:oppLineup,
    homeLineup: side==="home"?myLineup:oppLineup,
    ...row, // keep innings/other room fields
  };
  // Push so the other player can pick it up via poll
  await supa.upsertRoom(roomCode, {...shared, _writer:writerSide||side}).catch(e=>console.error("launchOnlineGame push failed:",e));
  // Set local state with perspective-specific rosters
...
function DebugPanel({log, onClear}){
  ...
  return(
    <div style={{position:"fixed",bottom:8,right:8,zIndex:9999}}>
      ...
      {show&&(
        <div style={{position:"fixed",bottom:40,right:8,width:320,maxHeight:280,
          background:"#0f172a",border:"1px solid #334155",borderRadius:8,
          overflowY:"auto",padding:8}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
            <span style={{fontSize:10,color:"#64748b",fontWeight:700}}>Online Sync Log</span>
            <button onClick={onClear}
              style={{background:"none",border:"none",color:"#475569",fontSize:10,cursor:"pointer"}}>
              Clear
            </button>
          </div>
          {log.length===0&&<div style={{fontSize:10,color:"#334155"}}>No events yet</div>}
          {log.map((l,i)=>(
            <div key={i} style={{fontSize:10,fontFamily:"monospace",
              color:l.includes("PUSH")?"#34d399":l.includes("APPLY")?"#60a5fa":l.includes("SKIP")?"#475569":"#94a3b8",
              marginBottom:2,borderBottom:"1px solid #1e293b",paddingBottom:2}}>
              {l}
            </div>
          ))}
        </div>
...
function Scoreboard({g,onBoxScore,onChangePitcher,onSoundToggle}){
  ...
  return(
    <>
      <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",alignItems:"center",gap:10,width:"100%",maxWidth:580,padding:"6px 10px 0",boxSizing:"border-box"}}>
        <div style={{background:awayBatting?aC.primary+"22":"#111827",border:`2px solid ${awayBatting?aC.primary:"#1e293b"}`,borderRadius:12,padding:"6px 10px",textAlign:"center"}}>
          <div style={{fontSize:26,fontWeight:900,lineHeight:1,color:awayBatting?aC.primary:"#f8fafc"}}>{g.score.away}</div>
        ...
        <div style={{background:!awayBatting?hC.primary+"22":"#111827",border:`2px solid ${!awayBatting?hC.primary:"#1e293b"}`,borderRadius:12,padding:"6px 10px",textAlign:"center"}}>
          <div style={{fontSize:26,fontWeight:900,lineHeight:1,color:!awayBatting?hC.primary:"#f8fafc"}}>{g.score.home}</div>
          <div style={{fontSize:10,color:"#64748b",textTransform:"uppercase",letterSpacing:1.5,marginTop:3}}>{g.teams.home}</div>
        </div>
      </div>
      <div style={{display:"flex",justifyContent:"center",gap:20,padding:"4px 10px",width:"100%",maxWidth:580,boxSizing:"border-box",alignItems:"center"}}>
...
function RollingScreen({g,onReveal}){
  ...
  const dice=g.lastDice||{batterRoll:1,pitcherRoll:1,luckRoll:1,luckMod:0,net:0,outcome:"out",outcomeLabel:"Out"}, batter=g.lastBatter, pitcher=g.lastPitcher;
  const aC=g.teamColors.away, hC=g.teamColors.home;
  ...
  const root={minHeight:"100vh",background:"linear-gradient(160deg,#0d0a1e 0%,#0a1628 50%,#0d1a0a 100%)",color:"#f8fafc",fontFamily:"'Inter','Segoe UI',sans-serif",display:"flex",flexDirection:"column",alignItems:"center",paddingBottom:8,overflowX:"hidden"};
  const card={background:"linear-gradient(180deg,#161f2e 0%,#111827 100%)",border:"1px solid #253046",borderRadius:12,padding:12,width:"100%",maxWidth:580,boxSizing:"border-box",margin:"0 8px",boxShadow:"0 2px 12px rgba(0,0,0,0.4)"};
...
function BoxScoreModal({g,onClose}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:100,padding:"10px",overflowY:"auto"}}>
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:16}}>
      <div style={{background:"#111827",border:"1px solid #1e293b",borderRadius:16,padding:20,width:"100%",maxWidth:400}}>
        ...
        <div style={{fontSize:12,color:"#64748b",marginBottom:6}}>Select a reliever from the bullpen:</div>
  ...
  return(
    <div style={root}>
      ...
      <div style={{...card,maxWidth:400,margin:"60px auto 0",textAlign:"center"}}>
        ...
        <div style={{fontSize:15,fontWeight:800,marginBottom:2}}>{g.topBottom==="top"?"▲":"▼"} Inning {g.inning}</div>
        <div style={{color:"#64748b",fontSize:13,marginBottom:10}}>{nowBatting} now batting</div>
...
function BaseballGame(){
  ...
  // Online mode — push state when it's our turn, track last pushed to avoid duplicate writes
  const lastPushedRef = useRef(null);
  const lastAppliedRef = useRef("");
  const[debugLog,setDebugLog]=useState([]);
  const addDebug=(msg)=>setDebugLog(prev=>[`${new Date().toLocaleTimeString()}: ${msg}`,...prev].slice(0,15));
  useEffect(()=>{
    if(g.mode!=="online"||!roomCode||onlineState!=="playing") return;
    if(!g._turn || g._turn !== onlineSide) return;
    // Avoid pushing the same state twice (phase+turn combo as key)
    const key = g.phase + g._turn + (g.count?.balls??0) + (g.count?.strikes??0) + (g.outs??0) + (g.pitchChosen||"") + (g.inning||0) + (g.batterIndex?.away||0) + (g.batterIndex?.home||0);
    addDebug(`🔍 WRITE CHECK [${onlineSide}] key:${key} lastKey:${lastPushedRef.current}`);
    if(lastPushedRef.current === key){ addDebug(`⏭ DEDUP skip`); return; }
    lastPushedRef.current = key;
    addDebug(`📤 PUSH [${onlineSide}] ${g.phase} _turn:${g._turn}`);
    supa.upsertRoom(roomCode, {...g, _writer:onlineSide}).catch(e=>console.error("push failed:",e));
  },[g,roomCode,onlineState,onlineSide]);

  // Online mode — poll every 1.5s for opponent state (reliable fallback to WebSocket)
  useEffect(()=>{
    ...
    const poll = setInterval(async()=>{
      try{
        ...
        if(!remote) return;
        // If remote shows playing status, always apply regardless of _writer (both need it)
        addDebug(`📥 POLL [${onlineSide}] phase:${remote?.phase||remote?.status} _turn:${remote?._turn} _writer:${remote?._writer}`);
        if(remote.status==="playing"&&remote.awayRoster&&remote.homeRoster&&onlineState!=="playing"){
          const side=onlineSide; // our side
          const oppSide=side==="away"?"home":"away";
          const myRoster=remote[`${side}Roster`]||{batters:[],pitchers:[]};
          const oppRoster=remote[`${oppSide}Roster`]||{batters:[],pitchers:[]};
          const myLineup=remote[`${side}Lineup`]||myRoster.batters;
          const oppLineup=remote[`${oppSide}Lineup`]||oppRoster.batters;
          setOnlineState("playing");
          setG(prev=>({
            ...remote,
            playerSide:side,
        ...
        // For gameplay state: ignore our own pushes
        if(!remote._writer||remote._writer===onlineSide){ addDebug(`⏭ SKIP own/_writer:${remote?._writer}`); return; }
        if(!remote._turn) return;
        addDebug(`✅ APPLY [${onlineSide}] ${remote.phase} _turn:${remote._turn} ps:${remote.playerSide}`);
        setG(prev=>({
          ...remote,
          ...
          mode:"online",
          // Preserve local-only state that shouldn't sync
          pitchGuess: prev.pitchGuess,
        }));
    ...
    let wsUnsub = null;
    try{
      wsUnsub = supa.subscribeRoom(roomCode,(remote)=>{
        if(!remote||!remote._writer) return;
        if(remote._writer===onlineSide) return; // our own push — ignore
        addDebug(`✅ WS APPLY [${onlineSide}] ${remote.phase} _turn:${remote._turn}`);
        setG(prev=>({
          ...remote,
          playerSide:   prev.playerSide,
          playerRoster: prev.playerRoster,
    ...
    } catch(e){ console.warn("WS subscribe failed, using poll only"); }
    unsubRef.current = ()=>{ clearInterval(poll); wsUnsub&&wsUnsub(); };
    return()=>{ clearInterval(poll); wsUnsub&&wsUnsub(); };
      ...
      supa.getRoom(roomCode).then(row=>{
        ...
        const poll=setInterval(async()=>{
          ...
          if(oppRoster&&oppRoster.batters&&oppRoster.batters.length>0){
            clearInterval(poll);
            const myRoster=row[`${side}Roster`]||{batters:[],pitchers:[]};
            ...
            launchOnlineGame(roomCode,row,side,totalInnings,myRoster,oppRoster,myLineup,oppLineup,setOnlineState,setG,side);
          }
        },2000);
    ...
    if(g.mode==="online"&&roomCode){
      ...
      supa.getRoom(roomCode).then(row=>{
        ...
        const poll=setInterval(async()=>{
          ...
          if(oppRoster&&oppRoster.batters&&oppRoster.batters.length>0){
            clearInterval(poll);
            const myRoster=row[`${playerSide}Roster`]||{batters:[],pitchers:[]};
            const myLineup=row[`${playerSide}Lineup`]||myRoster.batters;
            const oppLineup=row[`${oppSide}Lineup`]||oppRoster.batters;
            const totalInnings=row.innings||g.totalInnings||3;
            launchOnlineGame(roomCode,row,playerSide,totalInnings,myRoster,oppRoster,myLineup,oppLineup,setOnlineState,setG,playerSide);
          }
        },2000);
        setTimeout(()=>clearInterval(poll),300000);
      });
  ...
  function choosePitch(pitchId){ addDebug(`⚾ choosePitch [${onlineSide}] ${pitchId}`); setG(prev=>({...prev,pitchChosen:pitchId,phase:"batting",_turn:prev.topBottom==="top"?"home":"away"}));} // pitcher wrote this
  ...
  function chooseSwing(swingId){
    addDebug(`🏏 chooseSwing [${onlineSide}] ${swingId}`);
    setG(prev=>{
      try{
      const battingSide=prev.topBottom==="top"?"away":"home";
      const pitchingSide=prev.topBottom==="top"?"home":"away";
      const bRoster=(battingSide===prev.playerSide?prev.playerRoster:prev.aiRoster)||{batters:[],pitchers:[]};
      const pRoster=(pitchingSide===prev.playerSide?prev.playerRoster:prev.aiRoster)||{batters:[],pitchers:[]};
      const pIdx=prev.activePitcherIdx?.[pitchingSide]||0;
      const idx=prev.batterIndex?.[battingSide]||0;
      const lineupArr=(prev.lineup?.[battingSide])||[];
      const batter=lineupArr.length>0?lineupArr[idx%lineupArr.length]:(bRoster.batters.length>0?bRoster.batters[idx%bRoster.batters.length]:null);
      if(!batter){ addDebug(`❌ chooseSwing: no batter found! lineup:${lineupArr.length} roster:${bRoster.batters.length}`); return prev; }
      const rawPitcher=pRoster.pitchers?.[pIdx]||null;
      ...
      }catch(e){ addDebug(`❌ swing ERR: ${e.message} lineup:${JSON.stringify(prev?.lineup)} bRoster:${prev?.playerRoster?.batters?.length}`); return prev; }
    });
  }
  ...
  const pitchBtn=(color,selected)=>({background:selected?`linear-gradient(135deg,${color},${color}bb)`:"#141d2e",border:`2px solid ${selected?color:"#253046"}`,color:selected?"#fff":"#7c8fa8",borderRadius:8,padding:"7px 6px",fontWeight:700,fontSize:12,cursor:"pointer",flex:1,minWidth:60,boxShadow:selected?`0 2px 8px ${color}55`:"none"});
  ...
  const bRoster=(battingSide===g.playerSide?g.playerRoster:g.aiRoster)||{batters:[],pitchers:[]};
  const pRoster=(pitchingSide===g.playerSide?g.playerRoster:g.aiRoster)||{batters:[],pitchers:[]};
  const lineupArr=(g.lineup&&g.lineup[battingSide])||[];
  const _bIdx=(g.batterIndex&&g.batterIndex[battingSide])||0;
  const activeBatter=lineupArr.length?lineupArr[_bIdx%lineupArr.length]:(bRoster.batters.length?bRoster.batters[_bIdx%bRoster.batters.length]:null);
  const _pIdx=(g.activePitcherIdx&&g.activePitcherIdx[pitchingSide])||0;
  const rawActivePitcher=pRoster.pitchers.length?pRoster.pitchers[_pIdx]:null;
  const activePitcher=rawActivePitcher?pitcherFatigue(rawActivePitcher,(g.pitcherBF&&g.pitcherBF[pitchingSide])||0):null;
  const playerTeamColor=(g.teamColors&&g.teamColors[g.playerSide])||TEAM_PALETTE[0];

  // Game-phase controls visibility
  const canChangePitcher=playerIsPitching&&g.phase==="pitching";

  // ── MENU ──────────────────────────────────────────────────────────────────
  const debugOverlay=(g.mode==="online"||onlineState!=="idle")
  ...
  if(g.phase==="menu")return(
    <div style={root}>
      {debugOverlay}
      <div style={{...card,maxWidth:400,margin:"60px auto 0",textAlign:"center"}}>
        <div style={{background:"linear-gradient(135deg,#1e3a5f,#2d1b69,#1a3a2a)",borderRadius:12,padding:"14px 16px 12px",marginBottom:12}}>
          <div style={{fontSize:36,marginBottom:4}}>⚾</div>
          <div style={{fontSize:22,fontWeight:900,letterSpacing:-1,marginBottom:4,background:"linear-gradient(135deg,#60a5fa,#f59e0b,#34d399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Diamond Duel</div>
          <div style={{color:"#94a3b8",fontSize:13}}>Draft · Guess · Swing · Win</div>
        </div>
        <div style={{color:"#64748b",marginBottom:6,fontSize:14}}>Draft · Set Lineup · Guess the Pitch · Win</div>
        <div style={{background:"linear-gradient(135deg,#141d2e,#111827)",border:"1px solid #253046",borderRadius:12,padding:"12px 16px",marginBottom:14,textAlign:"left"}}>
    ...
    return(
      ...
      {debugOverlay}
        <div style={{...card,maxWidth:420,margin:"40px auto 0"}}>
          ...
          {g.mode==="vs-ai"&&(<><div style={lbl}>Play as</div><div style={{display:"flex",gap:10,marginBottom:12}}>{sideBtn("away","⚡","Away","Bats first")}{sideBtn("home","🏠","Home","Bats second")}</div></>)}
        ...
        <div style={{width:"100%",maxWidth:580,padding:"14px 16px 0",boxSizing:"border-box"}}>
          ...
          {isMyTurn ? (
            <div>
              ...
              {options.length===0 && (
                <div style={{color:"#475569",textAlign:"center",padding:"12px 0",fontSize:13}}>No eligible players available</div>
              )}
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {options.map(p=>{
                  ...
                  return(
                    <div key={p.id}
                      style={{background:"#0f172a",border:`2px solid ${eligible?"#1e293b":"#0f172a"}`,borderRadius:12,padding:"8px 10px",cursor:eligible?"pointer":"not-allowed",opacity:eligible?1:0.35,transition:"border-color 0.12s"}}
  ...
  // ── ROLLING ──────────────────────────────────────────────────────────────
  if(g.phase==="rolling"){
    // In online mode, only the batter resolves the dice — pitcher waits
    if(isOnline && !playerBatting){
      return(
        <div style={root}>
      {debugOverlay}
          ...
          <div style={card}>
            <div style={{color:"#64748b",textAlign:"center",padding:"16px 0"}}>⏳ Waiting for opponent to reveal dice…</div>
          </div>
        </div>
      );
    }
    return <RollingScreen g={g} onReveal={resolveOutcome}/>;
  }
    ...
    <div style={root}>
      ...
      <div style={card}>
        ...
        {playerIsPitching
          ...
          :<div style={{color:"#64748b",textAlign:"center",padding:"12px 0"}}>{isOnline?"⏳ Waiting for opponent to pitch…":"⚾ AI is winding up…"}</div>
        }
      </div>
    </div>
  );
  ...
  return(
    <div style={root}>
      ...
      <div style={card}>
        ...
        {playerIsBatting ? (()=>{
          ...
          return(
            <div>
              ...
              {/* Guess feedback */}
              <div style={{fontSize:10,color:"#475569",marginBottom:6,minHeight:14}}>
                {g.pitchGuess
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {SWING_TYPES.map(s=>(
                  <button key={s.id}
                    ...
                    onClick={()=>{ addDebug(`🖱 SWING BTN [${onlineSide}] ${s.id} isBatting:${playerIsBatting}`); chooseSwing(s.id); }}>
        ...
        :<div style={{color:"#64748b",textAlign:"center",padding:"12px 0"}}>{isOnline?"⏳ Waiting for opponent to swing…":"🤖 AI batter deciding…"}</div>}
      </div>
    </div>
  );
  }
    ...
    return(
      ...
      {debugOverlay}
        ...
        <div style={card}>
          ...
          {dice&&(
            <div style={{background:"#0f172a",border:"1px solid #1e293b",borderRadius:12,padding:14,marginBottom:6}}>
  ...
  if(g.phase==="between"){
    // In online mode: away always controls the between-inning Continue button
    // Home sees a waiting screen until away continues
    if(isOnline && g.playerSide!=="away"){
      return(
        <div style={root}>
      {debugOverlay}
          <Scoreboard g={g} onBoxScore={()=>setShowBoxScore(true)} onSoundToggle={soundOn?{on:true,fn:()=>{_soundEnabled=false;setSoundOn(false);}}:{on:false,fn:()=>{_soundEnabled=true;setSoundOn(true);}}}/>
          <div style={card}>
            <div style={{color:"#64748b",textAlign:"center",padding:"16px 0"}}>⏳ Waiting for opponent to continue…</div>
          </div>
        </div>
      );
    }
    return <BetweenInning g={g} showBoxScore={showBoxScore} setShowBoxScore={setShowBoxScore} onContinue={continuePlay} />;
  }
    ...
    return(
      ...
      {debugOverlay}
        <div style={{...card,maxWidth:500,margin:"30px auto 0"}}>
          ...
          <div style={{fontSize:12,color:"#64748b",marginBottom:10}}>Step {step} of 3</div>
            ...
            if(done){
              return(
                <div style={{textAlign:"center"}}>
                  ...
                  <div style={{fontSize:13,color:"#64748b",marginBottom:12}}>AI teams will auto-draft when their games are simmed.</div>
      ...
      {debugOverlay}
        <div style={{width:"100%",maxWidth:580,padding:"14px 16px 0",boxSizing:"border-box"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div>
              <div style={{fontWeight:900,fontSize:17}}>🏆 Playoffs</div>
          ...
          {/* Champion banner */}
          {isChampion&&champion&&(
            <div style={{background:"linear-gradient(135deg,#92400e,#d97706,#92400e)",borderRadius:12,padding:"10px",textAlign:"center",marginBottom:14}}>
              <div style={{fontSize:36,marginBottom:4}}>🏆</div>
          ...
          {playoffHubTab==="bracket"&&(
            <div>
              {Array.from({length:po.totalRounds}).map((_,ri)=>{
                ...
                return(
                  <div key={rNum} style={{marginBottom:10}}>
          ...
          {playoffHubTab==="stats"&&(
            <div>
              ...
              {statsList.length===0&&(
                <div style={{color:"#475569",textAlign:"center",padding:"12px 0",fontSize:13}}>
        ...
        <div style={{...card,maxWidth:380,margin:"80px auto 0",textAlign:"center"}}>
          ...
          {g._onlineCode&&(
            <div style={{background:"#0f172a",borderRadius:10,padding:"14px",
              ...
              marginBottom:6,fontFamily:"monospace"}}>{g._onlineCode}</div>
          )}
          <div style={{fontSize:12,color:"#475569",marginBottom:10}}>Room code: <b>{g._onlineCode||roomCode}</b></div>
        ...
        <div style={{...card,maxWidth:420,margin:"60px auto 0",textAlign:"center"}}>
          ...
          {onlineState==="idle"&&(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              {/* Team name */}
              <div style={{textAlign:"left"}}>
                <div style={lbl}>Your Team Name</div>
                <div style={{fontSize:20,fontWeight:900,color:TEAM_PALETTE[playerColorIdx].primary,marginBottom:6,letterSpacing:-0.5}}>{teamName}</div>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {TEAM_PALETTE.map((_,i)=>(
                    <div key={i} onClick={()=>setPlayerColorIdx(i)}
                      style={{width:32,height:32,borderRadius:"50%",background:TEAM_PALETTE[i].primary,
                        cursor:"pointer",border:playerColorIdx===i?"3px solid #f8fafc":"3px solid transparent",
                        boxShadow:playerColorIdx===i?`0 0 0 2px ${TEAM_PALETTE[i].primary}`:"none",
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button style={{...btn("#3b82f6"),width:"auto",padding:"10px 16px"}}
                  onClick={async()=>{
                    ...
                    setOnlineSide("home");
                    // Signal host that home has joined, include home team info
                    await supa.upsertRoom(roomInput,{...row,status:"home_joined",
                      homeName:teamName,homeColorIdx:playerColorIdx});
                    ...
                    setG({
                      ...
                      aiRoster:{batters:[],pitchers:[]},
                      lineup:{away:[],home:[]},
                      teams:{away:row.awayName||"Away",home:teamName},
                      teamColors:{away:TEAM_PALETTE[row.awayColorIdx||0],home:TEAM_PALETTE[playerColorIdx]},
          ...
          {onlineState==="creating"&&(
            <div style={{color:"#64748b",padding:"12px 0"}}>Creating room…</div>
          )}

          {onlineState==="joining"&&(
            <div style={{color:"#64748b",padding:"12px 0"}}>Joining room…</div>
          )}
          ...
          {onlineState==="show_code"&&(
            <div style={{textAlign:"center"}}>
              ...
              <div style={{background:"#0f172a",borderRadius:12,padding:"10px",
              ...
              <button style={btn("#E63946")} onClick={()=>{
                ...
                setG({
                  ...
                  aiRoster:{batters:[],pitchers:[]},
                  lineup:{away:[],home:[]},
                  teams:{away:teamName,home:"Home"},
                  teamColors:{away:TEAM_PALETTE[playerColorIdx],home:TEAM_PALETTE[(playerColorIdx+3)%10]},
    ...
    return(
      ...
      {debugOverlay}
        <div style={{...card,maxWidth:460,margin:"30px auto 0",textAlign:"center"}}>
          <div style={{background:winSide?`linear-gradient(135deg,${g.teamColors[winSide].primary}33,#111827)`:"#111827",borderRadius:12,padding:"20px 16px 14px",marginBottom:10}}>
            <div style={{fontSize:36,marginBottom:4}}>"🏆"</div>
            <div style={{fontSize:20,fontWeight:900,marginBottom:2,color:winSide?g.teamColors[winSide].primary:"#f8fafc"}}>`${winner} Win!`</div>
          </div>

          {/* Final score */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
            {["away","home"].map(s=><div key={s} style={{background:"#1e293b",borderRadius:10,padding:12,border:`2px solid ${g.teamColors[s].primary}`}}><div style={{fontSize:10,color:"#64748b",textTransform:"uppercase",letterSpacing:1.5}}>{g.teams[s]}</div><div style={{fontSize:30,fontWeight:900,color:g.teamColors[s].primary}}>{g.score[s]}</div></div>)}
          </div>

          {/* Headlines */}
          <div style={{background:"#111827",border:"1px solid #1e293b",borderRadius:10,padding:"10px 14px",marginBottom:6,textAlign:"left"}}>
          ...
          {/* MVP card */}
          {mvpEntry&&(
            <div style={{background:"#0f172a",border:"1px solid #f59e0b55",borderRadius:10,padding:"10px 14px",marginBottom:6,display:"flex",alignItems:"center",gap:12}}>
