...
async function launchOnlineGame(roomCode, row, side, totalInnings, myRoster, oppRoster, myLineup, oppLineup, setOnlineState, setG){
  ...
  await supa.upsertRoom(roomCode, shared).catch(e=>console.error("launchOnlineGame push failed:",e));
...
export default function BaseballGame(){
  ...
  useEffect(()=>{
    ...
    supa.upsertRoom(roomCode, g).catch(e=>console.error("push failed:",e));
  ...
  useEffect(()=>{
    ...
    const poll = setInterval(async()=>{
      try{
        ...
        if(!remote) return;
        // If remote has playing status and game state but we're still waiting, launch game
        ...
        if(remote._turn === onlineSide) return; // our own state — ignore
    ...
    try{
      wsUnsub = supa.subscribeRoom(roomCode,(remote)=>{
        ...
        if(remote._turn===onlineSide) return;
        setG(prev=>{
          if(prev._turn===onlineSide) return prev;
          return{
            ...remote,
            playerSide:   prev.playerSide,
            playerRoster: prev.playerRoster,
      ...
      supa.getRoom(roomCode).then(row=>{
        ...
        const poll=setInterval(async()=>{
          ...
          if(oppRoster&&oppRoster.batters&&oppRoster.batters.length>0){
            ...
            launchOnlineGame(roomCode,row,side,totalInnings,myRoster,oppRoster,myLineup,oppLineup,setOnlineState,setG);
          }
        },2000);
      ...
      supa.getRoom(roomCode).then(row=>{
        ...
        const poll=setInterval(async()=>{
          ...
          if(oppRoster&&oppRoster.batters&&oppRoster.batters.length>0){
            ...
            const oppLineup=row[`${oppSide}Lineup`]||oppRoster.batters;
            const totalInnings=row.innings||g.totalInnings||3;
            launchOnlineGame(roomCode,row,playerSide,totalInnings,myRoster,oppRoster,myLineup,oppLineup,setOnlineState,setG);
          }
        },2000);
        setTimeout(()=>clearInterval(poll),300000);
      });
