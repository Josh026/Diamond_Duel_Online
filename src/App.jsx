...
export default function BaseballGame(){
  ...
  useEffect(()=>{
    ...
    const poll = setInterval(async()=>{
      try{
        ...
        // Only apply if something meaningful changed
        // Include pitchChosen and inning in key so batting states are distinct
        const remoteKey = remote.phase + remote._turn + (remote.count?.balls??0) + (remote.count?.strikes??0) + (remote.outs??0) + (remote.pitchChosen||"") + (remote.inning||0) + (remote.batterIndex?.away||0) + (remote.batterIndex?.home||0);
        if(lastAppliedRef.current === remoteKey){ addDebug(`⏭ DEDUP already applied ${remoteKey}`); return; }
        lastAppliedRef.current = remoteKey;
        addDebug(`✅ APPLY [${onlineSide}] ${remote.phase} _turn:${remote._turn} myPS:${prev?.playerSide}`);
        setG(prev=>({
          ...remote,
  ...
  function chooseSwing(swingId){
    addDebug(`🏏 chooseSwing [${onlineSide}] ${swingId} phase:${g.phase} _turn:${g._turn} pitcher:${g.activePitcherIdx?.[pitchingSide]}`);
    try{
    setG(prev=>{
      const battingSide=prev.topBottom==="top"?"away":"home";
      const pitchingSide=prev.topBottom==="top"?"home":"away";
    ...
    }catch(e){ addDebug(`❌ chooseSwing ERROR: ${e.message}`); }
  }
  ...
  if(g.phase==="batting"){
    addDebug(`🎯 BATTING RENDER [${onlineSide}] isBatting:${playerIsBatting} isOnline:${isOnline} pBatting:${playerBatting} topBottom:${g.topBottom} ps:${g.playerSide} mode:${g.mode}`);
  return(
    <div style={root}>
      {debugOverlay}
      {showBoxScore&&<BoxScoreModal g={g} onClose={()=>setShowBoxScore(false)}/>}
...
class ErrorBoundary extends React.Component {
  ...
  render(){
    if(this.state.error) return(
      <div style={{background:"#0a0f1a",color:"#f8fafc",minHeight:"100vh",padding:20,fontFamily:"monospace",fontSize:12}}>
        <div style={{color:"#f87171",fontWeight:700,marginBottom:12}}>❌ Crash — share this log</div>
        <div style={{color:"#fca5a5",whiteSpace:"pre-wrap",wordBreak:"break-all"}}>{this.state.error}</div>
      </div>
    );
...
export default function App(){ return <ErrorBoundary><_OriginalApp/></ErrorBoundary>; }
